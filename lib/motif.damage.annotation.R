...
extract.pfm <- function (database.path, force.run = F) {
	...
	if (file.exists(gsub("meme", "PFM.list.rds", paste0("./cache/", basename(database.path)))) & force.run == F) {
		...
		motifs$length <- do.call(rbind, strsplit(grep("alength", meme.file, value=T), " "))[,6]
...
get.damage.scores.direct <- function(database.path, variants, force.pfm.extract = F) {
	...
	system(paste0(fimo.path, " --text --skip-matched-sequence --parse-genomic-coord ", database.path, " ref.seq.file.txt > motif.map.tmp.txt"), ignore.stderr = F)
	...
	if(!all(unique(mapped.motifs.chr$Motif) %in% names(motif.PFMs))) {
	...
	mapped.vars$seqnames <- as.character(mapped.vars$seqnames)
	...
	top.motif <- tapply(1:nrow(mapped.vars), mapped.vars$xHits, function(x) mapped.vars[x,"seqnames"][which.max(abs(mapped.vars$log2.damage.score)[x])])
	top.motif.name <- mapped.motifs.chr$Alt_motif_id[match(top.motif, mapped.motifs.chr$Motif)]
	...
	variants$top.motif[names(top.motif)] <- top.motif
	variants$top.motif[names(top.motif)] <- top.motif.name
